...
window.IX={
	...
	card_click:me=>{ // 打开详情弹层
		...
		`${['japan','european','domestic'].includes(IX.filters.category)?IX.wh:IX.mh}video/videodetails?mediaKey=${mk}`.get(_=>{
			...
			o.push(`<div VS>${episodes.length>20?`<toggle onclick='run("IX","fold_toggle",WI)(this)'>🌀</toggle>`:''}${episodes.sort((a,b)=>parseInt(b.episodeTitle.replace(/[\D]/g,'')-parseInt(a.episodeTitle.replace(/[\D]/g,'')))).map((_,i)=>`<div mk='${mk}' vid='${_.episodeId}' ${i>19?'style="display:none" ':''}onclick='run("IX","part_click",WI)(this)'>${_.episodeTitle}</div>`).join('')}</div>`)
...
grid:empty::before{content:attr(_,'————  🫧 数据为空！  ————');display:block;line-height:24vh;color:#ccc;font-size:16px;text-align:center;background:linear-gradient(90deg,var(--b) 25%,rgba(255,255,255,.7) 50%,rgba(255,255,255,.05) 75%);background-size:200% 100%;animation:sk 1.4s infinite;-webkit-animation:sk 1.4s infinite;transform:translateZ(0);-webkit-transform:translateZ(0);will-change:background-position}
...
modal-c>div>div[b]{margin-right:3px}
modal-c>div>div[b],modal-c>div b{font-weight:blod;font-size:13px}
