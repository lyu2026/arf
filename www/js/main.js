...
String.prototype.r=function(f,x){let r=fetch(this+`?_vv=${(new Date).vv()}`,{cache:'force-cache'});for(let k of x.split('.'))r=r.then(_=>_[k=='t'?'text':(k=='h'?'html':'json')]());r.then(f)};
...
D.head.append(
	...
	D.n('style',{},`:root{
		...
		line-break:anywhere;line-height:1;-webkit-overflow-scrolling:touch!important;-webkit-tap-highlight-color:none;
		overscroll-behavior:none!important;scroll-behavior:smooth!important;box-sizing:border-box;user-select:none;outline:none;
	}
	...
	modal-c>video{display:block;margin-top:8px;border-radius:4px;border-bottom:1px solid #555}
