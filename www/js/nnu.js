...
window.IX={
	...
	tab_click:(me,go=_=>true)=>{ // 筛选视频
			...
			if(key=='category'){
				...
				if(IX.filters.category!='?'){
					...
					const tt=`<div${''==IX.filters.type?' c':''} V='' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>全部</div>`+X.types.map(_=>{
						let x=_.split(':');return `<div${x[0]==IX.filters.type?' c':''} V='${x[0]}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>${x[1]}</div>`
					}).join('')
					const ta=`<div${''==IX.filters.area?' c':''} V='' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>全部</div>`+X.areas.map(_=>{
						let x=_.split(':');return `<div${x[0]==IX.filters.area?' c':''} V='${x[0]}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>${x[1]}</div>`
					}).join('')
					const ty=`<div${''==IX.filters.year?' c':''} V='' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>全部</div>`+X.years.map(_=>{
						let x=_.split(':');return `<div${x[0]==IX.filters.year?' c':''} V='${x[0]}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>${x[1]}</div>`
					}).join('')
					const ts=[{k:'',v:'按时间排序'},{k:'click',v:'按人气排序'},{k:'rating',v:'按评分排序'}].map(_=>`<div${_.k==(IX.filters.sort||'update')?' c':''} V='${_.k}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'>${_.v}</div>`).join('')
		...
		U.get(o=>{
			...
			o=o.$$('.lists-content li').map(_=>{
				...
				return IX.name_reg.test(N)?null:$O.node('div',{GC:'',I,N,onclick:'run("IX","card_click",WI)(this)',tabindex:'0'},`<img crossorigin='anonymous' src='${VCVR}' s='https://nnyy.in${cover}'/><score>${score}</score><title>${N}</title>`)
			}).filter(Boolean)
			o&&gbox.append(...o)
			go(true)
		},{},'html')
	...
	card_click:me=>{ // 打开详情弹层
		...
		const id=IX.id=me.ga('I'),videos='nnu_favorite_videos'.gc({}),mbox=$O.$('[MC]').html(`<sk pt30 f fv g12><sk f g12><sk q w20 h40></sk><sk q w10 h40></sk><sk q w33 h40></sk><sk q f1 h40></sk></sk><sk f g12><sk q w20 h40></sk><sk q f1 h40></sk></sk><sk x6 g16>${'<sk b h20></sk>'.repeat(6)}</sk><sk q r169></sk><sk f g20><sk b w40 h12></sk><sk f1></sk><sk b w h12></sk></sk><sk q r219></sk></sk>`)
		IX.curr={N:me.ga('N'),C:me.$('img').ga('s'),S:me.$('score').innerText}
		$O.$('[MT] [SC]').innerText=id in videos?'♡':'⊕'
		$O.body.sa('ns')
		$O.$('[G]').da('a')
		...
		`https://nnyy.in/${id.replace(/\./g,'/')}.html`.get(async _=>{
			...
			o.push(`<div><div><b>地区:</b>&emsp;<em tabindex='0'>${x.area}</em>&emsp;&emsp;&emsp;<b>年份:</b>&emsp;<em tabindex='0'>${x.year}</em></div></div>`)
			o.push(`<div T='director'${x.director.length>0?'':' hide'}><div b>导演: </div>${x.director.map(_=>`<div V='${_.trim()}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'><em>${_.trim()}</em></div>`).join('')}</div>`)
			o.push(`<div T='actor'${x.actor.length>0?'':' hide'}><div b>主演: </div>${x.actor.map(_=>`<div V='${_.trim()}' onclick='run("IX","tab_click",WI)(this)' tabindex='0'><em>${_.trim()}</em></div>`).join('')}</div>`)
			o.push(`${urls.length>14?`<div tg><div onclick='run("IX","fold_toggle",WI)(this)'>🌀</div></div>`:''}<div VS${urls.length>14?' x':''}>${urls.map((_,i)=>`<div ep='${_.ep}' onclick='run("IX","part_click",WI)(this)' tabindex='0'>${_.title}</div>`).join('')}</div>`)
			o.push(`<div SS></div>`)
			...
			o.push(`<div BF tabindex='0'>${x.brief}</div>`)
			mbox.html(o.join(''))
