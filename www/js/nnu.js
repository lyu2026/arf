...
window.IX={
	...
	tab_click:(me,go=_=>true)=>{ // 筛选视频
		...
		U.get(o=>{
			...
			o=o.$$('.lists-content li').map(_=>{
				...
				return IX.name_reg.test(N)?null:$O.node('grid-c',{I,N,onclick:'run("IX","card_click",WI)(this)'},`<img src='${IX.cover}' s='https://nnyy.in${cover}'/><score>${score}</score><title>${N}</title>`)
			}).filter(Boolean)
			o&&gbox.append(...o)
			go(true)
		},{},'html')
	...
	card_click:me=>{ // 打开详情弹层
		...
		`https://nnyy.in/${id.replace(/\./g,'/')}.html`.get(_=>{
